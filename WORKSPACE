...
for p in glob.glob("third_party/xla/**/BUILD*", recursive=True):
    ...
    open(p, "w").write("cc_library(".join(blocks))'""",
        ...
        "python3 -c 'import glob; [open(p, \"w\").write(open(p).read().replace(\"native.register_toolchains(\\\"@local_config_python//:py_toolchain\\\")\", \"# native.register_toolchains(\\\"@local_config_python//:py_toolchain\\\")\").replace(\"native.register_execution_platforms(\\\"@local_execution_config_platform//:platform\\\")\", \"# native.register_execution_platforms(\\\"@local_execution_config_platform//:platform\\\")\").replace(\"native.register_toolchains(\\\"@local_execution_config_python//:py_toolchain\\\")\", \"# native.register_toolchains(\\\"@local_execution_config_python//:py_toolchain\\\")\")) for p in glob.glob(\"**/*.bzl\", recursive=True)]'",
