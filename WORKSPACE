...
for p in glob.glob("third_party/xla/**/BUILD*", recursive=True):
    ...
    open(p, "w").write("cc_library(".join(blocks))'""",
        ...
        "sed -i 's/tf_vendored(name = \"tsl\"/# tf_vendored(name = \"tsl\"/g' tensorflow/workspace3.bzl",
        "sed -i 's/tf_vendored(name = \"tsl\"/# tf_vendored(name = \"tsl\"/g' third_party/xla/workspace4.bzl",
