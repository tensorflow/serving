...
TEST_F(ResourceUtilTest, MaxBound) {
  const auto lhs = CreateProto<ResourceAllocation>(R"(
    resource_quantities {
      resource {
        device: 'main'
        device_instance { value: 0 }
  ...
  const auto rhs = CreateProto<ResourceAllocation>(R"(
    resource_quantities {
      resource {
        device: 'main'
        device_instance { value: 0 }
  ...
  EXPECT_THAT(util_.Max(lhs, rhs), EqualsProto(R"(resource_quantities {
                               resource {
                                 device: 'main'
                                 device_instance { value: 0 }
                                 kind: 'processing'
