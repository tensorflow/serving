...
#ifndef TENSORFLOW_SERVING_CORE_VERSION_POLICY_H_
#define TENSORFLOW_SERVING_CORE_VERSION_POLICY_H_
...
// ServableStateSnapshots, should return the same result.
//
// If additional state is required to implement a Policy, such state shall be
// shared via ServableStateSnapshots. Depending on the kind of state, the most
// likely candidates for originating or tracking state are Sources or the
// Harness and Manager.
class VersionPolicy {
 public:
  ...
  virtual optional<ServableAction> GetNextAction(
      const std::vector<ServableStateSnapshot>& all_versions) const = 0;
};

inline bool operator==(const VersionPolicy::ServableAction& lhs,
...
#endif  // TENSORFLOW_SERVING_CORE_VERSION_POLICY_H_
