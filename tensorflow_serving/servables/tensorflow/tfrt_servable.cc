...
absl::Status TfrtSavedModelServable::PredictStreamed(
  ...
  std::string signature_name = request.model_spec().signature_name().empty()
      ...
      [&](absl::flat_hash_map<std::string, tensorflow::Tensor> outputs) {
        ...
        *response.mutable_model_spec() = model_spec;
        ...
        for (const auto& [output_key, output_tensor] : outputs) {
          ...
          // predict_response_tensor_serialization_option_ == kAsProtoField. The
          // proper way is to check serialize based on the value of
          // predict_response_tensor_serialization_option_.
  ...
  // The actual responses are passed through `response_callback`. The graph
  // should have no output tensors currently.
  PredictResponse response;
...
absl::Status TfrtSavedModelServable::GetModelMetadata(
  ...
  for (const auto& metadata_field : request.metadata_field()) {
    if (metadata_field == kSignatureDef) {
      ...
      response_model_spec->set_name(name());
