...
def tf_serving_workspace():
    ...
    http_archive(
        ...
        patch_cmds = [
            "python3 -c 'import glob; [(lambda s=open(p).read(): open(p, \"w\").write(s.replace(\"class btree_iterator : private btree_iterator_generation_info {\", \"class btree_iterator : private btree_iterator_generation_info {\\n  template <typename, typename, typename> friend class btree_iterator;\").replace(\"  bool operator==(const const_iterator &other) const {\", \"  template <typename N, typename R, typename P> bool operator==(const btree_iterator<N, R, P> &other) const { return node_ == other.node_ && position_ == other.position_; }\\n  bool operator==(const const_iterator &other) const {\").replace(\"  bool operator!=(const const_iterator &other) const {\", \"  template <typename N, typename R, typename P> bool operator!=(const btree_iterator<N, R, P> &other) const { return node_ != other.node_ || position_ != other.position_; }\\n  bool operator!=(const const_iterator &other) const {\").replace(\"  const_iterator internal_end(const_iterator iter) const {\", \"  const_iterator internal_end(iterator iter) const { return iter.node_ != nullptr ? iter : end(); }\\n  const_iterator internal_end(const_iterator iter) const {\").replace(\"  using iterator =\\n      typename btree_iterator<node_type, reference, pointer>::iterator;\", \"  using iterator = btree_iterator<node_type, reference, pointer>;\")))() for p in glob.glob(\"**/btree.h\", recursive=True)]'",
            "python3 -c 'import glob; [(lambda s=open(p).read(): open(p, \"w\").write(s.replace(\" protected:\\n  // Alias used for heterogeneous lookup functions.\", \" public:\\n  // Alias used for heterogeneous lookup functions.\")))() for p in glob.glob(\"**/btree_container.h\", recursive=True)]'",
        ],
        repo_mapping = {
