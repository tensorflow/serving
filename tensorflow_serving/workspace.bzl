...
def tf_serving_workspace():
    ...
    http_archive(
        ...
        patch_cmds = [
            "python3 -c 'import glob, re; [open(p, \"w\").write(re.sub(r\"using iterator = std::conditional_t<[\\s\\S]*?const_pointer>>;\", \"  using iterator = btree_iterator<normal_node, reference, pointer>;\", re.sub(r\"typename container_internal::template btree[^\\n]*?node_type\\s*\\*parent;\", \"    node_type *parent;\", open(p).read()))) for p in glob.glob(\"**/btree.h\", recursive=True)]'",
        ],
        repo_mapping = {
