...
def tf_serving_workspace():
    ...
    http_archive(
        ...
        patch_cmds = [
            "python3 -c 'import glob, re; [(lambda s=open(p).read(): (\"friend class btree_iterator;\" not in s and open(p, \"w\").write(re.sub(r\"std::is_same<btree_iterator<N, R, P>,\\s*iterator>::value\\s*&&\\s*std::is_same<btree_iterator,\\s*const_iterator>::value\", r\"std::is_same<std::remove_const_t<N>, std::remove_const_t<node_type>>::value && std::is_same<btree_iterator, const_iterator>::value\", re.sub(r\"std::is_same<btree_iterator<N, R, P>,\\s*const_iterator>::value\\s*&&\\s*std::is_same<btree_iterator,\\s*iterator>::value\", r\"std::is_same<std::remove_const_t<N>, std::remove_const_t<node_type>>::value && std::is_same<btree_iterator, iterator>::value && !std::is_same<iterator, const_iterator>::value\", re.sub(r\"(class btree_iterator : private btree_iterator_generation_info \\{)\", r\"\\1\\n public:\\n  template <typename, typename, typename> friend class btree_iterator;\", re.sub(r\"(bool operator==\\(const const_iterator &other\\) const \\{)\", r\"template <typename N, typename R, typename P> bool operator==(const btree_iterator<N, R, P> &other) const { return node_ == other.node_ && position_ == other.position_; }\\n  \\1\", re.sub(r\"(bool operator!=\\(const const_iterator &other\\) const \\{)\", r\"template <typename N, typename R, typename P> bool operator!=(const btree_iterator<N, R, P> &other) const { return node_ != other.node_ || position_ != other.position_; }\\n  \\1\", re.sub(r\"(const_iterator internal_end\\(const_iterator iter\\) const \\{)\", r\"const_iterator internal_end(iterator iter) const { return iter.node_ != nullptr ? iter : end(); }\\n  \\1\", s))))).replace(\"class btree_iterator_generation_info_disabled {\", \"class btree_iterator_generation_info_disabled {\\n public:\").replace(\"class btree_iterator_generation_info_enabled {\", \"class btree_iterator_generation_info_enabled {\\n public:\").replace(\"btree_iterator(const btree_iterator<N, R, P> other)  // NOLINT\", \"btree_iterator(const btree_iterator<N, R, P> &other)  // NOLINT\").replace(\"explicit btree_iterator(const btree_iterator<N, R, P> other)\", \"explicit btree_iterator(const btree_iterator<N, R, P> &other)\").replace(\"node_(other.node_),\\n        position_(other.position_) {}\", \"node_(const_cast<node_type*>(other.node_)),\\n        position_(other.position_) {}\")))))() for p in glob.glob(\"**/btree.h\", recursive=True)]'",
