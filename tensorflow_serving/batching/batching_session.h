...
// paddings[i].first - number of elements to add before existing elements in dimension i,
...
// Functor, which performs padding of given input tensor using specified padding signature.
// For example, given tensor of shape [1, 2, 3] and padding signature
// [[0, 0], [0, 2], [2, 2]] functor produces PaddingResult with padded_tensor of shape [1, 4, 7].
...
PaddingResult PadTensorOfSpecificType(const Tensor& tensor,
    ...
    switch (num_dims) {
        ...
        // (like in tensorflow/core/kernels/pad_op.cc)
            res = {Tensor(), errors::InvalidArgument("Only tensors with rank from 0 to 6 can be padded.")};
    }
    return res;
}
...
// For zeroth dimension max_dim_size is set to zero, because padding is not performed in this dimension.
...
struct BatchingSessionOptions {
  ...
  // but with unequal dimensions, so that all tensors of the same name have equal
  // dim sizes.
  //
  // For example: given input tensors of shapes [1, 500, 101], [2, 300, 101], [1, 400, 101]
  ...
  // These models typically accept variable-length inputs and when training them typical strategy
  // is to save sequence lengths for decoding and pad those variable-length dims to maximum in batch.
  // So, this flag is used to achieve similar behavior when serving with batching,
  // it is assumed that sequence lengths have already been saved.
