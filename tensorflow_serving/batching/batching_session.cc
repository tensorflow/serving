...
//////////
// Backward-compatibility layer for old APIs.
// TODO(b/33476643): Remove.

// A ServingSession implementation that wraps a BatchingSession that it
// constructs upon receiving its first Run() call (and deducing the signature
// to use for batching).
class SignatureInferringBatchingSession : public ServingSession {
 public:
  SignatureInferringBatchingSession(
      const BatchingSessionOptions& options,
      std::unique_ptr<Session> raw_session,
 ...
 private:
  const BatchingSessionOptions options_;
  const BatchingSessionSchedulerCreator scheduler_creator_;

  // Exactly one of these is populated at a given time. Initially,
  // 'raw_session_' is populated. Upon the first Run() call, 'batching_session_'
  // is created from 'raw_session_' (and 'raw_session_' is cleared).
  std::unique_ptr<Session> raw_session_;
  std::unique_ptr<Session> batching_session_;

  mutable mutex batching_session_creation_mu_;

  TF_DISALLOW_COPY_AND_ASSIGN(SignatureInferringBatchingSession);
};

SignatureInferringBatchingSession::SignatureInferringBatchingSession(
    const BatchingSessionOptions& options, std::unique_ptr<Session> raw_session,
    BatchingSessionSchedulerCreator batch_scheduler_creator)
    : options_(options),
      scheduler_creator_(batch_scheduler_creator),
      raw_session_(std::move(raw_session)) {}

Status SignatureInferringBatchingSession::Run(
    const std::vector<std::pair<string, Tensor>>& inputs,
    const std::vector<string>& output_tensor_names,
    const std::vector<string>& target_node_names,
    std::vector<Tensor>* outputs) {
  {
    mutex_lock l(batching_session_creation_mu_);
    if (batching_session_ == nullptr) {
      // This is the first Run() call. Set up 'batching_session_'.
      const TensorSignature signature =
          TensorSignatureFromRunArgs(inputs, output_tensor_names);
      TF_RETURN_IF_ERROR(
          CreateBatchingSession(options_, {{signature, scheduler_creator_}},
                                std::move(raw_session_), &batching_session_));
    }
  }
  return batching_session_->Run(inputs, output_tensor_names, target_node_names,
                                outputs);
}
...
Status CreateBatchingSession(
    ...
    std::unique_ptr<Session> session,
    std::unique_ptr<Session>* batching_session) {
  batching_session->reset(new SignatureInferringBatchingSession(
      options, std::move(session), batch_scheduler_creator));
  return Status::OK();
}
